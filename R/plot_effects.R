...
plot.fmlogit.margins = function(object,varlist=NULL,X=NULL,y=NULL, 
  ...
  if(is.null(group.x) & is.null(group.algebra)) group=NULL
  if(is.null(group.x)==F) {M.group = X[,group.x]; g.name = group.x}
  ...
  for(c in var_colNo){
    ...
    if(is.null(group)==F) temp.data = cbind(temp.data,as.factor(M.group))
    colnames(temp.data) = c(colnames(object$marg.list[[c]]),ag.name,g.name)
    for(i in 1:j){
      ...
      g <- g + theme(legend.title = element_text(colour="black"))+
